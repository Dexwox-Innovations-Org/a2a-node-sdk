...
[A2A Node SDK - v0.1.0](/api-reference/README) / [Modules](/api-reference/modules) / [Server](/api-reference/modules/Server) / A2AServer

# Class: A2AServer

[Server](/api-reference/modules/Server).A2AServer
...
• **new A2AServer**(`agentCard`, `requestHandler`, `contextMiddleware?`): [`A2AServer`](Server.A2AServer)
...
| :------ | :------ | :------ |
| `agentCard` | [`AgentCard`](/api-reference/interfaces/Core.AgentCard) | The agent card describing this server's capabilities |
| `requestHandler` | [`RequestHandler`](/api-reference/interfaces/Server.RequestHandler) | Handler for processing incoming requests |
