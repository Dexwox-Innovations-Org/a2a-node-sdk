...
[A2A Node SDK - v0.1.0](/api-reference/README) / [Modules](/api-reference/modules) / Server
...
- [A2AServer](/api-reference/classes/Server.A2AServer)
...
- [TaskManager](/api-reference/classes/Server.TaskManager)
...
| `error` | [`A2AError`](/api-reference/classes/Core.A2AError) | The error object to include in the response |
...
| Name | Type |
| :------ | :------ |
| `T` | extends [`Task`](/api-reference/interfaces/Core.Task) \| \{ `content`: `string` ; `format`: ``"plain"`` \| ``"markdown"`` ; `type`: ``"text"``  } \| \{ `content`: `string` \| `Uint8Array`\<`ArrayBuffer`\> ; `mimeType`: `string` ; `name`: `string` ; `size?`: `number` ; `type`: ``"file"``  } \| \{ `content`: `Record`\<`string`, `any`\> ; `schema?`: `string` ; `type`: ``"data"``  } \| \{ `content`: `string` ; `format`: ``"plain"`` ; `type`: ``"heartbeat"``  } \| `Record`\<`string`, `unknown`\> |
...
| :------ | :------ | :------ |
| `id` | `undefined` \| `string` \| `number` | The request ID from the original JSON-RPC request |
| `response` | [`A2AError`](/api-reference/classes/Core.A2AError) \| `T` | The response object or error to include |
